...
public class RecipeDAO {
   ...
   public static List<RecipeVO> chefRecipeData(Map map)
   {
	   List<RecipeVO> list=new ArrayList<RecipeVO>();
	   SqlSession session=null;
	   try
	   {
		   ...
		   session=ssf.openSession();
		   // 데이터 처리
		   list=session.selectList("chefRecipeData",map);
	   }catch(Exception ex)
	   {
		   ex.printStackTrace();
	   }
   ...
   public static int chefRecipeTotalPage(String chef)
   {
	   int total=0;
	   SqlSession session=null;
	   try
	   {
		   // 연결
		   session=ssf.openSession();
		   total=session.selectOne("chefRecipeTotalPage",chef);
	   }catch(Exception ex)
	   {
		   ex.printStackTrace();
	   }
